...
print('getting all the validation data')
paths = open("MURA-v1.1/valid_image_paths.csv", newline='')
images = []
...
study_to_n = {}
paths_to_training_images = open("MURA-v1.1/valid_image_paths.csv", newline='')
...
print('getting labels for validation data')
labels_csv = open("MURA-v1.1/valid_labeled_studies.csv", newline='')
labels = []
...
with open('validation_images', 'wb') as f:
    np.save(f, images_tensor)

# saving labels
print('saving labels')
with open('validation_labels', 'wb') as f:
    np.save(f, labels)
